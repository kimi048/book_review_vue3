...
const Auth = useAuth();
let {isLoggedin, username, id, isSignUpPage} = storeToRefs(Auth);
...
const handleSubmit = async() => {
      await Auth.handleSubmit(isSignUpPage.value,SignUpForm.value.username, SignUpForm.value.password);
      if(isLoggedin.value){
        router.push("/mypage");
      }
...
<template>
  ...
  <div class="container">
    ...
    <div class="mb-6 flex space-x-6">
        <button class="bg-indigo-700 font-semibold text-white py-2 px-4 rounded" v-on:click="handleSubmit">{{isSignUpPage ? "ユーザー作成" : "ログイン"}}</button>
        <button class="bg-blue-500 font-semibold text-white py-2 px-4 rounded" @click="isSignUpPage=!isSignUpPage">{{isSignUpPage ? "ログイン画面に切り替える" : "ユーザーを登録する"}}</button>
